...
使用前提：
- 已在 Schwab Developer Portal 註冊好 App，Callback/Redirect URI 設定為 http://localhost:8182
- 有該 App 的 Client ID / Client Secret

這是個人一次性使用的小工具，故意寫得很陽春（沒有做 HTTPS、state 驗證等強化），
...
def main():
    ...
    server = http.server.HTTPServer(("localhost", 8182), CallbackHandler)
    print("等待瀏覽器授權導回 http://localhost:8182 ...")
